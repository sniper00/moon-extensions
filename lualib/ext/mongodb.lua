...
local c = require "rust.mongodb"

local protocol_type = 24
...
local function operators(self, ...)
    local res = self.obj:operators(moon.id, moon.next_sequence(), self.op_name,  self.db_name, self.col_name, ...)
...
function M.connect(database_url, name)
    local res = moon.wait(c.connect(protocol_type, moon.id, moon.next_sequence(), database_url, name))
    if res.kind then
